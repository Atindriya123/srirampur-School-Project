...
export const POST = async (request) => {
  try {
    ...
    if (action === "create" && data) {
      ...
      const addMarksheet = await prisma.marksheet.create({
        data: {
      ...
      return NextResponse.json(addMarksheet);
    } else if (action === "delete" && data && data.id) {
      // Deletion logic
      const deleteMarksheet = await prisma.marksheet.delete({
        where: {
          id: data.id,
        },
      });
      ...
      const searchResult = await prisma.marksheet.findFirst({
        where: {
          rollno: rollno,
          dob: new Date(dob).toISOString(),
          sclass: sclass,
